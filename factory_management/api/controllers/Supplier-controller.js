...
const SupProUpdate = async (req, res, next) => {
  ...
  console.log("nic thiyano"); 
   ...
   supplier.findOne({nic : newNic},function(err,data){
    ...
    console.log(err)
    console.log("meka   aaa "+data)

      if(supId ==data._id){
        supplier.updateOne({_id:req.details.id},{$set:req.body},function(err){
          if(!err){
             res.send("Your Profile info was Updated....");
          }else{
            res.send(err);
          }
      ...
      res.send("Your NIC already exists. Please contact factory")
    }
...
const DeleteAcc = async (req, res, next) => {
  const supId = req.details.id;
  console.log(supId);
  supplier.deleteOne({_id:supId},function(err){
    if(err){
      res.send(err)
    }else{
      res.send("Sucess")
...
const ManagerSpDeleteAcc = async (req, res, next) => {
...
const Addcoco = async(req, res, next) => {
 ...
 getCoconut.updateOne({name:"Coconut"},{$set:data},function(err){
  if(!err){
...
const ManegerHome = async (req, res, next) => {
...
exports.ManagerSpDeleteAcc = ManagerSpDeleteAcc;
exports.Addcoco = Addcoco;
exports.ManegerHome = ManegerHome;
