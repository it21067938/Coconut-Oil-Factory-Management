...
function UpdateBankC() {
  ...
  const getBankC = () => {
    axios
      .get("http://localhost:8889/CreditBankLedger/get/" +id)
      .then((res) => {
  ...
  return (
    ...
    <div className='col-md-8 mt-4 mx-auto'>
        <h2 className='h4 mb-3 font-weight-normal'><center>Update Bank Credit</center></h2>
        ...
        onSubmit={(e) => {
                ...
                console.log("Bank credit updated", newBankC);
                axios
                  .put("http://localhost:8889/CreditBankLedger/update/" +id, newBankC)
                  .then(() => {
                    alert("Bank credit updated",refreshPage());
                  })
                  .catch((err) => {
                    alert(err);
                  });
            ...
            <div className='form-group' style={{marginBottom:'15px'}}>
                  <label style={{marginBottom:'5px'}} className="lable">Cheque No</label>
                  <input value={ChequeNo} type="text"className="form-control"name='description'placeholder='Enter cheque number'
                      onChange={(event)=>{
