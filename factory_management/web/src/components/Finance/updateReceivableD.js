...
function UpdateReceivableD() {
  ...
  const getReceivableD = () => {
    axios
      .get("http://localhost:8889/ReceivableDebit/getReceivableDById/" +id)
      .then((res) => {
...
<div>
                    ...
                    <div className='col-md-8 mt-4 mx-auto'>
                        <h2 className='h4 mb-3 font-weight-normal'><center>Update Receivable Debit</center></h2>
                        
                        <form className='needs-validation' 
                             onSubmit={(e) => {
                              e.preventDefault();
                    
                              const newReceivableD = {
                               Date,
                               Description,
                               Amount
                              };
                              console.log("Receivable debit updated", newReceivableD);
                              axios
                                .put("http://localhost:8889/ReceivableDebit/update/" +id, newReceivableD)
                                .then(() => {
                                  alert("Receivable debit updated",refreshPage());
                                })
                                .catch((err) => {
                                  alert(err);
                                });
...
export default UpdateReceivableD;